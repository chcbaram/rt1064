...
static status_t SD_WaitWriteComplete(sd_card_t *card)
{
    ...
    do
    {
        ...
        if (kStatus_Success != error)
        {
            SDMMC_LOG("\r\nError: send CMD13 failed with host error %d, response %x", (int)error, (int)command.response[0U]);
            break;
        }
...
static status_t SD_SendWriteSuccessBlocks(sd_card_t *card, uint32_t *blocks)
{
    ...
    {
        SDMMC_LOG("\r\nError: send ACMD13 failed with host error %d, response %x", (int)error, (int)command.response[0U]);
    }
    else
    {
...
static status_t SD_SendRca(sd_card_t *card)
{
    ...
    {
        SDMMC_LOG("\r\nError: send CMD3 failed with host error %d, response %x", (int)error, (int)command.response[0U]);
    }
...
static status_t SD_SwitchFunction(sd_card_t *card, uint32_t mode, uint32_t group, uint32_t number, uint32_t *status)
{
    ...
    if ((kStatus_Success != error) || ((command.response[0U]) & SDMMC_R1_ALL_ERROR_FLAG))
    {
        SDMMC_LOG("\r\n\r\nError: send CMD6 failed with host error %d, response %x", (int)error, (int)command.response[0U]);
    }
...
static status_t SD_SendScr(sd_card_t *card)
{
    ...
    if ((kStatus_Success != error) || ((command.response[0U]) & SDMMC_R1_ALL_ERROR_FLAG))
    {
        SDMMC_LOG("\r\nError: send ACMD51 failed with host error %d, response %x", (int)error, (int)command.response[0U]);
    }
    else
    {
...
static status_t SD_SelectFunction(sd_card_t *card, uint32_t group, uint32_t function)
{
    ...
    {
        SDMMC_LOG("\r\nError: current card not support function %d", (int)function);
        return kStatus_SDMMC_NotSupportYet;
    }
    ...
    {
        SDMMC_LOG("\r\nError: switch to function %d failed", (int)function);
...
static status_t SD_SetDataBusWidth(sd_card_t *card, sd_data_bus_width_t width)
{
    ...
    if ((kStatus_Success != error) || ((command.response[0U]) & SDMMC_R1_ALL_ERROR_FLAG))
    {
        SDMMC_LOG("\r\nError: send ACMD6 failed with host error %d, response %x", (int)error, (int)command.response[0U]);
    }
...
static status_t SD_SendCsd(sd_card_t *card)
{
    ...
    else
    {
        ...
        SDMMC_LOG("\r\nError: send CMD9(get csd) failed with host error %d, response %x", (int)error, (int)command.response[0U]);
    }
...
static status_t SD_ApplicationSendOperationCondition(sd_card_t *card, uint32_t argument)
{
    ...
    while (i--)
    {
        ...
        if (kStatus_Success != error)
        {
            SDMMC_LOG("\r\nError: send ACMD41 failed with host error %d, response %x", (int)error, (int)command.response[0U]);
            return kStatus_SDMMC_TransferFailed;
        }
...
static status_t SD_SendInterfaceCondition(sd_card_t *card)
{
    ...
    do
    {
        ...
        if (kStatus_Success != error)
        {
            SDMMC_LOG("\r\nError: send CMD8 failed with host error %d, response %x", (int)error, (int)command.response[0U]);
        }
        else
        {
...
static status_t SD_Read(sd_card_t *card, uint8_t *buffer, uint32_t startBlock, uint32_t blockSize, uint32_t blockCount)
{
    ...
    {
        SDMMC_LOG("\r\nError: read with parameter, block size %d is not support", (int)blockSize);
                         ...
                         uint32_t *writtenBlocks)
{
    ...
    {
        SDMMC_LOG("\r\nError: write with parameter, block size %d is not support", (int)blockSize);
        return kStatus_SDMMC_CardNotSupport;
    }
    ...
    *writtenBlocks  = blockCount;
    ...
    if (error != kStatus_Success)
    {
        ...
        SDMMC_LOG("\r\nWarning: write failed with block count %d, successed %d", (int)blockCount, (int)(*writtenBlocks));
    }
...
static status_t SD_Erase(sd_card_t *card, uint32_t startBlock, uint32_t blockCount)
{
    ...
    if (kStatus_Success != error)
    {
        SDMMC_LOG("\r\nError: send CMD32(erase start) failed with host error %d, response %x", (int)error, (int)command.response[0U]);
        return kStatus_SDMMC_TransferFailed;
    }
    ...
    if (kStatus_Success != error)
    {
        SDMMC_LOG("\r\nError: send CMD33(erase end) failed with host error %d, response %x", (int)error, (int)command.response[0U]);
        return kStatus_SDMMC_TransferFailed;
    }
    ...
    if (kStatus_Success != error)
    {
        SDMMC_LOG("\r\nError: send CMD38(erase) failed with host error %d, response %x", (int)error, (int)command.response[0U]);
        return kStatus_SDMMC_TransferFailed;
    }
